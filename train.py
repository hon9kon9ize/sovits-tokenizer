...
def run(rank, n_gpus, hps):
    ...
    train_dataset = AudioSpeakerLoader(hps.data)  ########
    ...
    # if rank == 0:
    #     eval_dataset = AudioSpeakerLoader(hps.data.validation_files, hps.data, val=True)
    ...
    et_p = list(map(id, net_g.enc_p.encoder_ssl.parameters()))
    mrte_p = list(map(id, net_g.enc_p.mrte.parameters()))
    base_params = filter(
        lambda p: id(p) not in et_p + mrte_p and p.requires_grad,
        net_g.parameters(),
    )

    # et_p=net_g.enc_p.encoder_ssl.parameters()
    ...
    optim_g = torch.optim.AdamW(
        ...
        [
            {
                "params": net_g.enc_p.encoder_ssl.parameters(),
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
        for batch_idx, (
            ...
            for test in [0, 1]:
                y_hat, mask, *_ = (
                    generator.module.infer(ssl, spec, spec_lengths, test=test)
                    if torch.cuda.is_available()
                    else generator.infer(ssl, spec, spec_lengths, test=test)
                )
