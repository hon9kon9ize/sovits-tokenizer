...
class AudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, hparams, val=False):
        ...
        self.audiopaths_sid = list(names4 & names5)
        tmp = self.audiopaths_sid
        leng = len(tmp)
        min_num = 100
        if leng < min_num:
            self.audiopaths_sid = []
            for _ in range(max(2, int(min_num / leng))):
                self.audiopaths_sid += tmp
        ...
        print("wav_data_len:", len(self.audiopaths_sid))

        audiopaths_sid_new = []
        lengths = []
        skipped_dur = 0
        for audiopath in tqdm(self.audiopaths_sid):
            ...
            if 54 > duration > 0.6 or self.val:
                audiopaths_sid_new.append([audiopath])
                lengths.append(size // (2 * self.hop_length))
            else:
                skipped_dur += 1
                continue

        print("total left: ", len(audiopaths_sid_new))
        assert len(audiopaths_sid_new) > 1  # 至少能凑够batch size，这里todo
        self.audiopaths_sid = audiopaths_sid_new
        self.lengths = lengths

    def get_audio_speaker_pair(self, audiopath_sid):
        (audiopath,) = audiopath_sid
        try:
