...
class SynthesizerTrn(nn.Module):
    ...
    def forward(self, ssl, y, y_lengths):
        ...
        x, m_p, logs_p, y_mask = self.enc_p(quantized, y_lengths, ge)
    ...
    def infer(self, ssl, y, y_lengths, test=None, noise_scale=0.5):
        ...
        x, m_p, logs_p, y_mask = self.enc_p(quantized, y_lengths, ge, test=test)
    ...
    @torch.no_grad()
    def decode(self, codes, refer, noise_scale=0.5, speed=1):
        def get_ge(refer):
            ge = None
        ...
        x, m_p, logs_p, y_mask = self.enc_p(quantized, y_lengths, ge, speed)
        z_p = m_p + torch.randn_like(m_p) * torch.exp(logs_p) * noise_scale
